...
function logValueSnapshot() {
  ...
  for (const s of getActiveSets()) {
    ...
    const totalInSet = data.filter(row => row[2]).length - 1;  // skip blank rows
    let owned = 0, value = 0;
  ...
  const values = new Array(newHeaders.length).fill("");
  Object.entries(row).forEach(([key, val]) => {
    const col = newHeaders.indexOf(key);
    if (col !== -1) values[col] = val;
  });
